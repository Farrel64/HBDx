...
.controller('mapCtrl', function($scope, $state, $cordovaGeolocation) {
  ...
  $scope.pubs = [{
    ...
    secondDrinkType: "Cocktail",
      position: {lat: 44.84057, lng: -0.58149},
    rating: [4.2]
  }, {
    id: 2,
    name: "Camelot",
    ...
    position: {lat: 44.83237, lng: -0.57107},
    rating: [3.8]
  }, {
    id: 3,
    name: "Titi Twister",
    ...
    position: {lat: 44.83175, lng: -0.56963},
    rating: [4.6, 3.9]
  }, ];
  ...
  $cordovaGeolocation.getCurrentPosition(options).then(function(position){
    ...
    google.maps.event.addListenerOnce($scope.map, 'idle', function(){
      for(var i = 0; i<$scope.pubs.length; i++)
      {
        ...
        var id = $scope.pubs[i].id;
        marker.addListener('click', function gotoPub() {
          $state.go('pub', ({"pubId":id}));
        });
      }
